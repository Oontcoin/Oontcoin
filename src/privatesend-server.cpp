...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Oontcoin related functionality
...
/*
    ...
    allow endless transaction that would bloat Oontcoin and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Oontcoin specific functionality
