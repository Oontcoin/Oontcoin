...
const char * const BITCOIN_CONF_FILENAME = "oontcoin.conf";
const char * const BITCOIN_PID_FILENAME = "oontcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "oontcoin" is a composite category enabling all Oontcoin-related debug output
                if(ptrCategory->count(std::string("oontcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Oontcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Oontcoin
    // Mac: ~/Library/Application Support/Oontcoin
    // Unix: ~/.oontcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Oontcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Oontcoin";
#else
    // Unix
    return pathRet / ".oontcoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty oontcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override oontcoin.conf
