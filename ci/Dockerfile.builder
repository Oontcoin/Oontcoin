...
# oontcoin_hash
RUN git clone https://github.com/oontcoinpay/oontcoin_hash
RUN cd oontcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} oontcoin
RUN useradd -u ${USER_ID} -g oontcoin -s /bin/bash -m -d /oontcoin oontcoin
...
RUN mkdir /oontcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /oontcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /oontcoin-src
